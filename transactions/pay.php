        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              <h4 class="py-3 mb-4"><span class="text-muted fw-light"><a href="../index.php">Home</a> / Account /</span> Deposit Fund to Supplier Account</h4>
...
<div class="col-md-6 col-lg-6">
  <div class="card mb-4">
    <p class="card-header">Deposit Fund to Supplier Account</p>
    <div class="card-body">
      <div class="form-floating mb-2">
        <input type="text" id="mobile" class="form-control" />
        <label for="mobile">Supplier Phone number</label>
      </div>
	  
	  <div class="form-floating form-floating-outline mb-2">
       <input type="text" class="form-control" id="customerPublicKey" placeholder="Stellar Public Key" readonly  disabled />
       <label for="customerPublicKey">Supplier Stellar Public Key</label>
      </div>
      ...
      <div class="form-floating form-floating-outline mb-2">
        <input type="text" class="form-control" id="customerName" placeholder="Supplier Name" readonly disabled />
        <label for="customerName">Supplier Name</label>
      </div>

      <div>
        <button type="button" class="btn btn-primary form-control" onclick="sendToAgent()">Deposit / Pay</button>
      </div>
    </div>
  </div>
</div>
...
document.getElementById('mobile').addEventListener('input', async (e) => {
    ...
    let code = input.value.trim().replace(/\D/g, ''); // Remove non-digits

    // Auto-format to 0XXXXXXXXX (10 digits starting with 0)
    if (code.length > 10) code = code.slice(0, 10);
    if (code.length > 0 && !code.startsWith('0')) code = '0' + code;
    input.value = code;

    // Clear fields initially
    const nameInput = document.getElementById('customerName');
    const keyInput = document.getElementById('customerPublicKey');
    nameInput.value = '';
    keyInput.value = '';

    // Stop if not exactly 10 digits
    if (code.length !== 10) {
        return;
    }

    try {
        const response = await fetch(`getCustomer.php?mobile=${encodeURIComponent(code)}`);
        const data = await response.json();

        console.log("Fetched supplier data:", data); // 🔍 For debugging

        if (!data || typeof data !== 'object') {
            nameInput.value = "Invalid response";
            return;
        }

        if (data.error) {
            nameInput.value = data.error.includes("not found") ? "Supplier not found" : data.error;
            return;
        }

        if (data.username) nameInput.value = data.username;
        if (data.stellarPublicKey) keyInput.value = data.stellarPublicKey;

        if (data.username && data.stellarPublicKey) {
            calculateWithdrawTZStoXLM(); // Trigger calculation only if valid
        }

    } catch (error) {
        console.error('Fetch Error:', error);
        nameInput.value = "Connection error";
    }
});
...
document.getElementById('amountDeposit').addEventListener('input', calculateWithdrawTZStoXLM);
  ...
  function sendToAgent() {
    const customerCode = document.getElementById('mobile').value.trim();
...
async function sendXLM(destination, amount) {
  ...
  try {
    ...
    const response = await fetch('save_deposit.php', {
      method: 'POST',
      headers: {
        ...
        <table class="table align-items-center mb-0" id="dataTable" width="100%" cellspacing="0">
          <thead>
            <tr>
              ...
              <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Supplier_Name</th>
          ...
          <tbody>
            ...
            try {
              ...
              if (isset($_SESSION['username'])) {
                  ...
                  $stmt = $conn->prepare("SELECT * FROM deposits WHERE user = :user ORDER BY created_at DESC");
